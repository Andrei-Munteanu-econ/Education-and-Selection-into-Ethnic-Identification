# =====================================================================
# Bootstrap estimation of the structural model under UNIFORM heterogeneity.
# Produces:  results_uniform.rds (data frame of estimates over education
#            groups x bootstrap replicates)
# Inputs:    objects defined by 00_main_structural.R (data, data_sum, f(),
#            model_uniform, S/S1/S2); run after sourcing that script
# Summary:   Runs f(i, j, "uniform") in parallel over every education group i and
...
setwd(wd_code)
start_time <- Sys.time()
# Estimate the UNIFORM model for each (i, j) in parallel and row-bind the results
